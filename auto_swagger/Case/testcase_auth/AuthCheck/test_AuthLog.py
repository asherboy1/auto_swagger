...
@ddt
class Test_AuthLog(unittest.TestCase):
    # 读取配置文件
    file_name = config.read('excel', 'cases_auth_authcheck')
    tmp = FindDir('auth', 'authcheck')
    tmppath = tmp.finddir()
    ...
    # Excel 操作
    xls = ExcelHandler(file_path, 'Auth_AuthLog')
