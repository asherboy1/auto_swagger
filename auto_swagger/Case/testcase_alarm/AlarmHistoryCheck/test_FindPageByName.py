...
@ddt
class Test_FindPageByName(unittest.TestCase):
    # 读取配置文件以及测试用例路径信息
    file_name = config.read('excel', 'cases_alarm_historycheck')
    tmp = FindDir('alarm', 'historycheck')
    tmppath = tmp.finddir()
    ...
    # Excel 操作
    xls = ExcelHandler(file_path, 'Alarm_FindPageByName')
    ...
    def test_alarm(self, data_item):
        ...
        print(type(data_item['params']),type(eval(data_item['params'])),self.headers)
