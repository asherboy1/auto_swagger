...
@ddt
class Test_FindByEquipmentModelId(unittest.TestCase):
    # 读取配置文件以及测试用例路径信息
    file_name = config.read('excel', 'cases_alarm_define')
    tmp = FindDir('alarm', 'define')
    tmppath = tmp.finddir()
    ...
    # Excel 操作
    xls = ExcelHandler(file_path, 'Alarm_FindPageByQuery')
